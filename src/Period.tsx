...
export const Period = ({
  ...
  const [monthButtons, setMonthButtons] = useState<NodeListOf<Element> | null>(null);
  const [monthButtons2, setMonthButtons2] = useState<NodeListOf<Element> | null>(null);
  ...
  const handleChange = useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    event.currentTarget.setAttribute('aria-checked', event.currentTarget.classList.contains('Mui-selected') ? 'true' : 'false')

    const monthName = event.currentTarget.ariaLabel || '';
    const monthNumber = dates[monthName]

    const newPeriod: Period = { year: currentYear, month: monthNumber, label: monthName }

    if (periods) {
      setPeriods((prev) => {
        if (prev.some((period) => period.year === newPeriod.year && period.month === newPeriod.month)) {
          return prev.filter((period) => period.year !== newPeriod.year || period.month !== newPeriod.month)
        } else {
  ...
  const handleChange2 = useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
    event.currentTarget.classList.toggle('Mui-selected')
    event.currentTarget.setAttribute('aria-checked', event.currentTarget.classList.contains('Mui-selected') ? 'true' : 'false')
    ...
    const newPeriod: Period = { year: currentYear2, month: monthNumber, label: monthName }
  ...
  useEffect(() => {
    resetButtons()
    const currentMonths = periods.filter((period) => period.year === currentYear)
    ...
    const currentMonths2 = periods.filter((period) => period.year === currentYear2)
    const currentMonthsNames2: string[] = [];
    currentMonths2.forEach((month) => currentMonthsNames2.push(month.label))

    if (monthButtons2) {
      monthButtons2.forEach((button) => {
        if (button.ariaLabel && currentMonthsNames2.includes(button.ariaLabel)) {
          button.classList.add('Mui-selected')
        } 
      })
    }
  }, [currentYear, currentYear2, periods, resetButtons, monthButtons, monthButtons2])

  const handleComplete = useCallback(() => {
    const dates = periods.map((period) => new Date(period.year, period.month - 1, 1));
    ...
    <Paper sx={{ p: 2, fontFamily: 'arial', borderRadius: 4, position: 'relative'}}>
      <Stack flexDirection={'column'} justifyContent={'space-between'}  sx={{ minHeight: 520 }}>
        <Box>
          ...
          <Box sx={{ mb: 2 , borderRadius: 2, backgroundColor: '#fff', minHeight: 100,  }}>
            {years.length > 0 && years.map((year) => <PeriodGroup key={year} year={year} periods={periods}/>)}
          </Box>
        </Box>
        
        <Stack flexDirection={'row'} justifyContent={'flex-end'} gap={1}>
          <Button variant="outlined" sx={{ width: 140 }} onClick={handleReset} disabled={periods.length <= 0}>Сбросить</Button>
          <Button variant="contained" sx={{ width: 140 }} onClick={handleComplete}>Подтвердить</Button>
        </Stack>
      </Stack>
    </Paper>
  );
