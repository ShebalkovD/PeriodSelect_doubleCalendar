...
import { Calendar } from 'MonthSelector/Calendar';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import type { JSX } from '@emotion/react/jsx-runtime';
import { FastPeriod } from 'MonthSelector/FastPeriod';
...
years.forEach((year) => {
  for (let i = 0; i < 12; i++) {
    ...
    const label = `${MONTH_NAMES[i]} ${year}`;
...
export const MonthSelector = (): JSX.Element => {
  ...
  const handleFastPeriodClick = useCallback(
  ...
  // Сохранение id быстрого периода при закрытии PopUp, если выбраны значения
  useEffect(() => {
